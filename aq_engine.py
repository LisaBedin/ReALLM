...
from src.aq import QuantizedWeightlqDora as QuantizedWeight
from src.aq import QuantizedWeightDynamo
...
class AQEngine(nn.Module):
    ...
    def __init__(self, layer: nn.Linear,
                 accumultor_dtype: torch.dtype = torch.float64,
                 layer_name: str = '',
                 quantizor_type: str = 'dora'):
        super().__init__()
        self.layer = layer
        ...
        self.nsamples = 0
        self.quantizor_type = quantizor_type
    ...
    def quantize(self, *, args: Namespace, verbose: bool = True) -> QuantizedWeight:
        ...
        if self.quantizor_type in ['classic', 'dora']:
            ...
            self.quantized_weight = QuantizedWeightDynamo(
                XTX=self.XTX.to(device=self.device, dtype=torch.float32),
                reference_weight=self.layer.weight.detach().to(device=self.device, dtype=torch.float32),
                reference_name=self.layer_name,
                out_group_size=args.out_group_size,
                in_group_size=args.in_group_size,
                num_codebooks=args.num_codebooks,
                nbits_per_codebook=args.nbits_per_codebook,

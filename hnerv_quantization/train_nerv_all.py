...
def main():
    ...
    parser.add_argument('--vid', type=str, default='q0_512', help='video id',)
    ...
    parser.add_argument('--crop_list', type=str, default='-1', help='video crop size',)
    ...
    parser.add_argument('--enc_strds', type=int, nargs='+', default=[2,2,2,2,2], help='stride list for encoder')
    parser.add_argument('--enc_dim', type=str, default='64_16', help='enc latent dim and embedding ratio')
    parser.add_argument('--modelsize', type=float,  default=7.2, help='model parameters size: model size + embedding parameters')
    ...
    parser.add_argument("--conv_type", default=['convnext', 'quantpshuffel'], type=str, nargs="+",
    ...
    parser.add_argument('-e', '--epochs', type=int, default=20000, help='Epoch number')
    ...
    parser.add_argument('--eval_freq', type=int, default=100, help='evaluation frequency,  added to suffix!!!!')
    parser.add_argument('--quant_model_bit', type=int, default=16, help='bit length for model quantization')
    parser.add_argument('--quant_embed_bit', type=int, default=16, help='bit length for embedding quantization')
...
def evaluate(model, full_dataloader, local_rank, args, 
    ...
    for model_ind, cur_model in enumerate(model_list):
        ...
        results_list = [torch.stack(v_list, dim=1).mean(1).cpu() if len(v_list) else torch.zeros(1)

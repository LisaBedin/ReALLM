...
@torch.no_grad()
def quantize_aq(
    model: PreTrainedModel, dataloader: Iterable, args: Namespace, # quantizor_type: str,
    ...
    model_q = lora_utils.load_peft_model(model_q, args.checkpoint_dir)
    print(model_q)
    ...
    for layer_index in range(len(layers)):
        ...
        if run_validation:
            ...
            if len(args.devices) == 1:
                ...
                print(val_inps[0].shape, val_outs[0].shape)
                update_outs(layer_q, val_inps[0], val_outs[0], compute_mse=not args.skip_out_loss, **forward_args)
            else:
                update_outs_parallel(
                    args.devices, layer_q, val_inps, val_outs, compute_mse=not args.skip_out_loss, **forward_args
                )

        if len(args.devices) == 1:
            assert len(inps) == len(outs) == 1
            print(inps[0].shape, outs[0].shape)
            update_outs(layer, inps[0], outs[0], compute_mse=False, **forward_args)
        else:
            update_outs_parallel(
                args.devices, layer, inps, outs, compute_mse=False, **forward_args
            )
        ...
        results_path = os.path.join(args.checkpoint_dir, 'finetune_bloc')
        os.makedirs(args.checkpoint_dir, exist_ok=True)
        layer_save_path = os.path.join(results_path, f"{layer_index}.pth")
        print(f"Saving layer {layer_index}... to {layer_save_path}")
        torch.save(layer_q.state_dict(), layer_save_path)

        if len(args.devices) == 1:
            assert len(inps) == len(outs) == 1
            out_losses = update_outs(layer_q, inps[0], outs[0], compute_mse=not args.skip_out_loss, **forward_args)
        else:
            out_losses = update_outs_parallel(
                args.devices, layer_q, inps, outs, compute_mse=not args.skip_out_loss, **forward_args
            )
        ...
        if run_validation:
            if len(args.devices) == 1:
                ...
                out_val_losses = update_outs(
                    layer_q, val_inps[0], val_outs[0], compute_mse=not args.skip_out_loss, **forward_args
                )
            else:
        ...
        inps, outs = outs, inps  # the input of the next layer is the output of the current quantized layer (not the output of the original layer)
        if run_validation:
    ...
    print("=====================\nFinal stats:")
    torch.save(vars(args), os.path.join(results_path, "args.pt"))
    save_not_quantized_weights(model, results_path)
    torch.save(model_q.state_dict(), os.path.join(results_path, 'full_model.pth'))
...
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--checkpoint_dir",
        type=str,
        default='/mnt/data/lisa/LLM_results/VQ_dora3_8B_2b_bucket4_rank64', # VQ_dora_2b_bucket4_rank64',
    ...
    if args.wandb:
        ...
        args.exp_name = (
            ...
            + args.checkpoint_dir.split('/')[-1] + '_finetune_bloc'
        )
    ...
    print("\n============ Quantizing model... ============")
    model_q = quantize_model(model, args)
    ...
    for dataset in datasets:
        ...
        perplexity_eval(model_q, testloader, args)

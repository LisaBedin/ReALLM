...
from src.aq_utils import create_normal_float_scheme, svd_decomposition, blockwise_absmax, quantize_with_scheme

from models import allocation_utils as allocation_utils_LLaMA
from models.lq_utils import maybe_sparsify_or_quantize
...
class QuantizedLinear(nn.Module):
    ...
    def _forward(self, input: torch.Tensor):
        ...
        # TODO: we dequantize the weight here. Can we do something else ?

    def _cheap_forward(self, input: torch.Tensor):
        weight = self.quantized_weight.Q
        print(input.shape, weight.shape)
        L1, L2 = self.quantized_weight.L1, self.quantized_weight.L2

        norm_reco_weight = torch.linalg.vector_norm(weight + (L1 @ L2), ord=2, dim=0)
        input /= norm_reco_weight.unsqueeze(0) * self.quantized_weight.dora.unsqueeze(0)
        return ((weight @ input.T).T + input @ L2.T @ L1.T)

    def forward(self, input: torch.Tensor):
        # if self.quantized_weight.weight_type == 'dynamo':
        ...
        return self._cheap_forward(input[0]).unsqueeze(0)

class QuantScheme(NamedTuple):
    values: torch.Tensor
    boundaries: torch.Tensor

class QuantizedWeightDynamo(nn.Module):
    def __init__(
            self,
            *,
            reference_weight: torch.Tensor,
            reference_name: str,
            # TODO: instead of having the tensor we should have name, submodule...
            nbits_per_codebook: int = 3,
        ...
        self.lora_num_ranks = 64
        self.lora_dropout = 0.0
        self.lora_config = 'lora-lpq'
        self.lora_model_name = 'nf3'
        self.weight_type = 'dynamo'

        codebooks_values, codebooks_boundaries = create_normal_float_scheme(
            num_bits=nbits_per_codebook,
        ...
        qconfig_dict, sensitivity_dict = (
            ...
            .create_qconfig_and_sensitivity_dict_LLaMA(
                identifier=self.lora_model_name))
        qconfig = qconfig_dict[reference_name]

        with torch.no_grad():
            last_error = reference_weight.new_tensor(float("inf"))
            Q = torch.zeros_like(reference_weight)
            for _ in range(40):
                ...
                Q = maybe_sparsify_or_quantize(
                    reference_weight - L1 @ L2,
                    qconfig=qconfig)
                # breakpoint()
                A_ = L1 @ L2 + Q
                error = torch.linalg.norm(reference_weight - A_, ord="fro")
            ...
            scale_dora = torch.linalg.vector_norm(Q + L1 @ L2, ord=2, dim=0)
            # self.scales = nn.Parameter(scales_dq, requires_grad=False) # do we need scales_dq ?
            self.Q = Q # TODO do we need to put requires_grad=False somewhere ?
        self.codebooks_values = nn.Parameter(
            codebooks_values, requires_grad=False
        )  # [num_codebooks, codebook_size, out_group_size, in_group_size]
        self.codebooks_boundaries = nn.Parameter(codebooks_boundaries, requires_grad=False)
        # self.codes = nn.Parameter(codes, requires_grad=False)  # [num_out_groups, num_in_groups, num_codebooks]
        self.L1 = nn.Parameter(L1, requires_grad=True)  # [LOWRANK
        self.L2 = nn.Parameter(L2, requires_grad=True)  # [LOWRANK
        self.dora = nn.Parameter(scale_dora, requires_grad=True)
    ...
    def forward(self, selection: Union[slice, ellipsis, torch.Tensor] = ...):
        ...
        """
        # weight = self.get_codebooks()[self.codes[selection]] * self.get_scales()[selection]
        weight = self.Q
        # norm_reco_weight = torch.linalg.vector_norm(weight + self.L1@self.L2, ord=2, dim=0)
        norm_reco_weight = torch.linalg.vector_norm(weight + (self.L1 @ self.L2)[selection], ord=2, dim=0)
        # return (weight + self.L1@self.L2) /norm_reco_weight * self.dora
        return (weight + (self.L1 @ self.L2)[selection]) / norm_reco_weight * self.dora
    ...
    def estimate_nbits_per_parameter(self) -> float:
        ...
        matrix_store = num_parameters * self.nbits_per_codebook
        lowrank_store = 8 * 2 * self.out_features
        scale_store = self.out_features * 16  # (+nb bits in qscheme values/boundaries)
        return (matrix_store + lowrank_store + scale_store) / num_parameters



class QuantizedWeightlqDora(nn.Module):
    def __init__(
            self,
            *,
            reference_weight: torch.Tensor,
        ...
        self.out_features, self.in_features = reference_weight.shape
        self.nbits_per_codebook = nbits_per_codebook
        self.weight_type = 'dora'

        codebooks_values, codebooks_boundaries = create_normal_float_scheme(
            num_bits=nbits_per_codebook,
            device=reference_weight.device)
        qscheme_object = QuantScheme(codebooks_values, codebooks_boundaries)

        with torch.no_grad():
            Q = torch.zeros_like(reference_weight)

            for _ in range(40):
                L1, L2 = svd_decomposition(reference_weight - Q,
                                           randomized=True,
                                           num_ranks=64,
                                           num_oversampling=10)
                _, scales_dq = blockwise_absmax(reference_weight - L1 @ L2, 8, "fp32", 64, 256)
                codes, Q = quantize_with_scheme(reference_weight - L1 @ L2,
                                             qscheme=qscheme_object,
                                             scales_q=scales_dq,
                                             scales_dq=scales_dq)
                # Q = codebooks_values[codes] * scales_dq

            scale_dora = torch.linalg.vector_norm(Q + L1 @ L2, ord=2, dim=0)
            self.scales = nn.Parameter(scales_dq, requires_grad=False)
        self.codebooks_values = nn.Parameter(
            codebooks_values, requires_grad=False
        )  # [num_codebooks, codebook_size, out_group_size, in_group_size]
        self.codebooks_boundaries = nn.Parameter(codebooks_boundaries, requires_grad=False)

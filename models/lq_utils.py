...
def maybe_sparsify_or_quantize(
    ...
    if fake is True:
        ...
        return quantization_utils_2.quantize(
            A,
            method=method, # "blockwise-permute", # "blockwise-nf",  # TODO: method="blockwise-permute"
            qconfig=qconfig)
...
def lowrank_quantized_sparse_decomposition(
    ...
    for _ in range(num_iterations):
        ...
        if num_iterations == 1:
           return L1, L2, Q, None
        # print('ctensor', Q._container.ctensor.shape)
        #if num_iterations == 1:
        #  breakpoint()
        A_ = L1 @ L2 + Q
        if W is None:
        ...
        if error > last_error:
            del L1, L2, Q
            torch.cuda.empty_cache()
            return prev_L1, prev_L2, prev_Q, None
        last_error = error
        prev_L1, prev_L2, prev_Q = L1, L2, Q
    del L1, L2, Q
    torch.cuda.empty_cache()
    return prev_L1, prev_L2, prev_Q, None

    ...
    for name, submodule in model.named_modules():
        ...
        if isinstance(submodule, lora.LoraLayer):
            ...
            try:
                ...
                qweight = state_dict[f"{name}.base_layer.weight"]
...
def transform_lora_layers(
    ...
    for name, submodule in model.named_modules():
        ...
        # This implicitly assumes that `LoraLayer`
        # do not include `LoraLayer` within the module.
        if isinstance(submodule, lora.LoraLayer): #  and ('gate' in name): #  or 'up' in name or 'down' in name):
            ...
            if lpq is False:
                ...
                n_it = num_iterations
                # breakpoint()
                if 'mlp' in name and len(model.model.model.layers) >= 40 and model_name == 'nf2':
                    n_it = 1
...
def transform_lora_layer_lpq(
    ...
    # 2. B = L1 / sqrt(s)
    # ```
    scale_sqrt = math.sqrt(module.scaling['default'])
    module.lora_A['default'].weight.copy_(L2 / scale_sqrt)
    module.lora_B['default'].weight.copy_(L1 / scale_sqrt)
    # TODO dora
    if module.lora_magnitude_vector is not None:
        scale_dora = torch.linalg.vector_norm(Q + L1 @ L2, ord=2, dim=1)
        module.lora_magnitude_vector['default'] = torch.nn.Parameter(scale_dora, requires_grad=module.lora_magnitude_vector.default.requires_grad)
...
def assert_lora_Linear_layer(
    ...
    if (module.bias is not None or
        module.lora_A['default'].bias is not None or
        module.lora_B['default'].bias is not None):
        raise ValueError
...
def replace_weight_(
    ...
    if isinstance(new_weight, tensor_container_utils.QuantizedTensor):
        ...
        try:
            ...
            module.base_layer.weight = torch.nn.Parameter( # TODO: how to do that ?
                new_weight,
                requires_grad=module.base_layer.weight.requires_grad)
    else:
        module.weight.copy_(new_weight)


def transform_lora_adapters_nf8(model: PeftModelForCausalLM, method: str = 'blockwise-nf') -> None:
    if not isinstance(model, PeftModelForCausalLM):
        raise TypeError
